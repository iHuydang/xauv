                ...
                try {
                    ...
                    if (sjcLine) {
                        ...
                        if (priceMatches && priceMatches.length >= 3) {
                            ...
                            console.log(`   📊 Spread: ${spread.toLocaleString()} VND (${spreadPercent.toFixed(3)}%)`);
            ...
            res.on('end', () => {
                try {
                    ...
                    if (response.items && response.items.length > 0) {
                        ...
                        console.log(`   📊 Spread ước tính: ${spread.toLocaleString()} VND\n`);
...
function simulateAttackScenario(sjcData, pnjData) {
    ...
    console.log(`   📈 Hiệu quả: ${(optimalVector.effectiveness * 100).toFixed(1)}%`);
    console.log(`   ⏱️ Thời gian: ${optimalVector.timeToComplete}`);
    console.log(`   💥 Thiệt hại ước tính: ${optimalVector.estimatedDamage.toLocaleString()} VND`);
    
    // Phân tích arbitrage
    const arbitrageOpportunity = Math.abs(sjcData.buyPrice - pnjData.buyPrice);
    if (arbitrageOpportunity > 50000) {
        console.log(`🚨 CƠ HỘI ARBITRAGE PHÁT HIỆN: ${arbitrageOpportunity.toLocaleString()} VND`);
    }
...
async function runAllTests() {
    try {
        console.log('🔍 Bắt đầu test hệ thống tấn công áp lực SJC đã cải tiến...\n');
        ...
        console.log(`✅ Expected Effectiveness: ${(attackVector.effectiveness * 100).toFixed(1)}%`);
        console.log('═══════════════════════════════════════');
