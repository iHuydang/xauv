      ...
      res.on("end", () => {
        try {
          ...
          if (goldData.price) {
            ...
            console.log(
              `   📊 Change 24h: ${goldData.ch || 0} (${goldData.chp || 0}%)`,
            );
            console.log(`   📈 High: $${goldData.high_price || "N/A"}`);
            console.log(`   📉 Low: $${goldData.low_price || "N/A"}`);
            console.log(
              `   ⏰ Last Update: ${goldData.date || new Date().toISOString()}`,
            );
            ...
            console.log(
              `   🔄 Simulated Bid/Ask: $${bid.toFixed(2)}/$${ask.toFixed(2)}`,
            );
            ...
            resolve({
              ...
              spread,
              liquidityLevel:
                spread < goldData.price * 0.001 ? "high" : "medium",
            });
          } else {
  ...
  for (const target of targets) {
    try {
      ...
      const data = await new Promise((resolve, reject) => {
        ...
        req.on("error", reject);
        req.on("timeout", () =>
          reject(new Error(`${target.name} request timeout`)),
        );
...
function parseSJCData(html) {
  try {
    ...
    return {
      ...
      spreadPercent,
      liquidityLevel:
        spread < 30000 ? "high" : spread < 80000 ? "medium" : "low",
...
function testAttackVectorAnalysis(worldGold, vietnamGold) {
  ...
  console.log(
    `   💰 Arbitrage Gap: ${arbitrageOpportunity.toLocaleString()} VND`,
  );
...
async function runCompleteSystemTest() {
  try {
    ...
    console.log(
      `✅ Attack Analysis: ${attackAnalysis ? attackAnalysis.riskLevel : "N/A"} risk level`,
    );
    console.log(`✅ Telegram Bot: ${telegramTest.capabilities} commands ready`);
    console.log(
      `✅ System Integration: ${integrationTest.integrationPoints} points verified`,
    );
    console.log("═══════════════════════════════════════");
    ...
    if (attackAnalysis && attackAnalysis.vulnerabilityScore > 60) {
      ...
      console.log(
        `   Vectors: ${attackAnalysis.recommendedVectors.join(", ")}`,
      );
      console.log(
        `   Arbitrage: ${attackAnalysis.arbitrageOpportunity.toLocaleString()} VND`,
      );
    }

    console.log("\n🎯 Enhanced Gold Trading System is FULLY OPERATIONAL!");
    console.log(
      "🔗 All components integrated with Vietnamese language support",
    );
    ...
    const mockResults = {
      worldGold: { price: 2680.5, changePercent: 1.2 },
      vietnamGold: [
        {
          source: "SJC",
          buyPrice: 79500000,
