...
const quickLiquidityScan = {
  ...
  scanXAUUSDFallingBuy: async () => {
    ...
    const results = {
      ...
      liquidity: {
        buy: {
          ...
          signal: priceChange < -3 ? 'VERY_STRONG' : priceChange < -2 ? 'STRONG' : 'MODERATE',
    ...
    console.log(`📉 Price: $${currentPrice.toFixed(2)} (${priceChange.toFixed(2)}%)`);
    console.log(`💰 BUY Liquidity: ${results.liquidity.buy.volume.toLocaleString()}`);
    console.log(`🎯 Buy Signal: ${results.liquidity.buy.signal}`);
    console.log(`📊 Buy Pressure: ${results.liquidity.buy.buyPressure.toFixed(1)}%`);
  ...
  monitorXAUUSDFallingBuy: async (intervalSeconds = 30) => {
    console.log(`🔄 Starting XAUUSD falling buy-side monitoring (${intervalSeconds}s interval)`);
    ...
    const monitor = setInterval(async () => {
      ...
      if (result.priceChange < -1) {
        ...
        if (consecutiveFalls >= 3) {
          ...
          console.log(`   🎯 Target: $${(result.currentPrice + 20).toFixed(2)}`);
  ...
  analyzeXAUUSDBuyDepth: async () => {
    ...
    analysis.depthAnalysis.liquidityScore = Math.min(totalVolume / 1000000 * 100, 100);
    analysis.depthAnalysis.marketImpact = Math.max(0, 100 - analysis.depthAnalysis.liquidityScore);
    
    console.log(`📈 Total Buy Volume: ${totalVolume.toLocaleString()}`);
    console.log(`💰 Average Buy Price: $${analysis.depthAnalysis.averagePrice.toFixed(2)}`);
    console.log(`🎯 Liquidity Score: ${analysis.depthAnalysis.liquidityScore.toFixed(1)}/100`);
    console.log(`⚡ Market Impact: ${analysis.depthAnalysis.marketImpact.toFixed(1)}%`);
