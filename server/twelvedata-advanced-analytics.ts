...
export class TwelveDataAdvancedAnalytics extends EventEmitter {
  ...
  private generateRecommendation(analysis: TechnicalAnalysis): 'BUY' | 'SELL' | 'HOLD' {
    let score = 0;
    ...
    for (const symbolA of goldSymbols) {
      for (const symbolB of goldSymbols) {
        if (symbolA !== symbolB && symbolA === goldData.symbol) {
          ...
          opportunity.spread = Math.abs(opportunity.priceA - opportunity.priceB);
          opportunity.spreadPercent = (opportunity.spread / opportunity.priceA) * 100;

          if (opportunity.spreadPercent > 0.1) {
            opportunity.potentialProfit = opportunity.spread * 100; // Per 100 units
            opportunity.recommendedAction = opportunity.priceA > opportunity.priceB ? `Buy ${symbolB}, Sell ${symbolA}` : `Buy ${symbolA}, Sell ${symbolB}`;
            opportunity.riskLevel = opportunity.spreadPercent > 0.5 ? 'high' : 'medium';
  ...
  private generateGoldSignals(goldData: any): void {
    ...
    const signal = {
      ...
      type: 'gold_signal',
      action: goldData.changePercent > 0.5 ? 'BUY' : goldData.changePercent < -0.5 ? 'SELL' : 'HOLD',
  ...
  private async performPeriodicAnalysis(): Promise<void> {
    ...
    try {
      ...
      console.log(`📊 Periodic analysis complete - ${this.activeAlerts.size} alerts, ${this.arbitrageOpportunities.size} opportunities`);
  ...
  // Public methods
  public getTechnicalAnalysis(symbol?: string): TechnicalAnalysis | Map<string, TechnicalAnalysis> {
    if (symbol) {
