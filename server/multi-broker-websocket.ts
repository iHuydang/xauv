...
export class MultiBrokerWebSocketManager extends EventEmitter {
  ...
  async startAllConnections(): Promise<void> {
    ...
    console.log(
      `✅ Connected to ${this.connections.size} brokers out of ${this.brokerConfigs.size} configured`,
    );
  ...
  private parseOandaMessage(message: any, broker: string): MarketData | null {
    if (message.type === "PRICE" && message.instrument) {
      return {
        ...
        spread:
          parseFloat(message.asks[0]?.price || "0") -
          parseFloat(message.bids[0]?.price || "0"),
  ...
  private scheduleReconnection(brokerId: string, config: BrokerConfig): void {
    ...
    if (brokerId === "exness") {
      console.log(
        `🔴 Auto-reconnection disabled for ${config.name} - manual reconnection required`,
      );
      this.emit("broker_failed", {
        brokerId,
        broker: config.name,
    ...
    console.log(
      `🔄 Scheduling reconnection to ${config.name} in ${delay}ms (attempt ${attempts + 1})`,
    );
  ...
  async placeOrder(
    ...
    if (!ws || !config || ws.readyState !== WebSocket.OPEN) {
      console.error(
        `❌ Cannot place order: ${config?.name || brokerId} not connected`,
      );
      return null;
    }
  ...
  private async connectProvider(provider: any): Promise<void> {
    try {
      ...
      if (this.isDNSProblematic(url.hostname)) {
        console.log(
          `⚠️ Potential DNS issue detected for ${url.hostname}, using fallback...`,
        );
  ...
  private handleDNSError(provider: any): void {
    ...
    if (alternative) {
      console.log(
        `🔄 Switching ${provider.name} to alternative endpoint: ${alternative}`,
      );
