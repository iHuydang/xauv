...
router.post('/tradermade/liquidity/sweep', async (req, res) => {
  try {
    ...
    if (force_sweep) {
      await tradermadeIntegration.executeLiquiditySweep(symbol.toUpperCase(), price.bid, price.ask);
    ...
    res.json({
      ...
      data: {
        ...
        attack_analysis: {
          ...
          attack_feasibility: priceDeviation > 0.05 ? 'high' : priceDeviation > 0.02 ? 'medium' : 'low',
          recommended_action: priceDeviation > 0.05 ? 'strong_attack' : 'monitor',
  ...
  try {
    ...
    if (currentDeviation >= target_deviation || attack_type === 'forced') {
      ...
      const attackSignal = {
        ...
        recommendation: attack_type === 'strong' || currentDeviation > 0.05 ? 'strong_attack' : 'moderate_attack'
      };
      ...
      res.json({
        ...
        data: {
          ...
          gold_price: goldPrice.mid,
          suggestion: 'Wait for higher price deviation or use attack_type=forced'
        }
      });
    }
  } catch (error) {
    ...
    res.json({
      ...
      data: {
        ...
        liquidity_operations: {
          ...
          order_types: liquidityOrders.reduce((acc, order) => {
  ...
  try {
    ...
    res.json({
      ...
      data: {
        ...
        expected_outcome: 'Coordinated pressure on Vietnam gold market using international liquidity',
...
router.post('/tradermade/test/manipulation', async (req, res) => {
  try {
    ...
    const baseManipulation = strength === 'strong' ? 0.0005 : strength === 'medium' ? 0.0003 : 0.0001;
