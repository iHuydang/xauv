...
export class VietnamGoldTradingIntegration extends EventEmitter {
  ...
  public async processXAUUSDOrder(
    ...
    console.log(
      `🇻🇳 Tương đương: ${vietnamGoldWeight.toFixed(2)} gram = ${vietnamGoldTael.toFixed(3)} lượng vàng SJC`,
    );
  ...
  private async executeVietnamGoldTrade(
    order: VietnamGoldOrder,
  ): Promise<void> {
    console.log(
      `⚡ Thực hiện giao dịch vàng SJC: ${order.side.toUpperCase()} ${order.vietnamGoldTael.toFixed(3)} lượng`,
    );
    ...
    console.log(
      `📊 Chi tiết: ${order.side.toUpperCase()} ${order.vietnamGoldTael.toFixed(3)} lượng @ ${order.priceVND.toLocaleString()} VND/lượng`,
    );
  }
  ...
  private async recordSJCTransaction(order: VietnamGoldOrder): Promise<void> {
    ...
    console.log(
      `⚖️ Khối lượng: ${sjcRecord.weight_tael.toFixed(3)} lượng (${sjcRecord.weight_gram.toFixed(2)} gram)`,
    );
    console.log(
      `💰 Tổng tiền: ${sjcRecord.total_amount_vnd.toLocaleString()} VND`,
    );
  ...
  public async executeTradingOrder(
    ...
    console.log(
      `📈 ${side.toUpperCase()} ${lotSize} oz XAUUSD @ $${executionPrice}`,
    );
  ...
  public getConversionInfo(lotSize: number, currentPriceUSD: number): any {
    ...
    const sjcPriceVND =
      currentPriceUSD *
      this.conversionRates.usdToVnd *
      this.conversionRates.sjcPremium;
...
export const vietnamGoldTradingIntegration =
  new VietnamGoldTradingIntegration();
