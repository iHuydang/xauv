...
router.post("/twelvedata/unsubscribe", (req, res) => {
  try {
    ...
    const results = symbols.map((symbol) => ({
      symbol,
      unsubscribed: twelveDataIntegration.unsubscribeFromSymbol(
        symbol.toUpperCase(),
      ),
    }));
...
router.get("/twelvedata/analytics/technical/:symbol?", (req, res) => {
  try {
    const { symbol } = req.params;
    const analysis = twelveDataAdvancedAnalytics.getTechnicalAnalysis(
      symbol?.toUpperCase(),
    );
...
router.get("/twelvedata/analytics/arbitrage", (req, res) => {
  try {
    const opportunities =
      twelveDataAdvancedAnalytics.getArbitrageOpportunities();
