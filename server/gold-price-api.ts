...
export class GoldPriceAPI extends EventEmitter {
      ...
      try {
        ...
        if (
          ...
          const goldPrice: GoldPriceData = {
            ...
            timestamp: Date.now(),
            change_24h: response.data.change
              ? parseFloat(response.data.change)
              : undefined,
            change_percent_24h: response.data.change_pct
              ? parseFloat(response.data.change_pct)
              : undefined,
          };
      ...
      try {
        ...
        if (response.data && response.data.price) {
          ...
          if (goldPrice.change_24h) {
            console.log(
              `📊 24h Change: ${goldPrice.change_24h > 0 ? "+" : ""}${goldPrice.change_24h} (${goldPrice.change_percent_24h}%)`,
            );
          }
  ...
  async getCompleteGoldData(): Promise<{
    gold: GoldPriceData | null;
    exchange: ExchangeRateData | null;
  }> {
