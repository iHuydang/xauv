...
router.post("/high-volume-sjc/create-orders", async (req, res) => {
  try {
    ...
    const orderIds =
      await highVolumeSJCOrderSystem.createHighVolumeOrders(totalOrders);
  ...
  try {
    ...
    res.json({
      ...
      data: {
        ...
        brokerSpreadDistribution:
          "Real profits distributed to broker dealing desk",
        physicalGoldDelivery:
          "Coordinated with Vietnamese financial institutions",
      },
...
router.get("/high-volume-sjc/by-institution", async (req, res) => {
  try {
    const institutionSummary =
      highVolumeSJCOrderSystem.getOrdersByInstitution();
    ...
    res.json({
      ...
      data: {
        ...
        physicalGoldCoordination:
          "Each institution responsible for physical gold delivery to SJC",
        brokerSpreadDistribution:
          "Real profits from spread distributed to broker after settlement",
      },
    ...
    res.json({
      ...
      data: {
        ...
        deliveryCoordination:
          "Financial institutions coordinate physical gold transport to SJC locations",
...
router.post("/high-volume-sjc/create-and-execute", async (req, res) => {
  try {
    ...
    console.log(
      `🚀 Creating and executing ${totalOrders} high-volume SJC orders...`,
    );
    ...
    res.json({
      ...
      data: {
        ...
        physicalGoldDelivery: {
          ...
          conversion: "1 lot = 82.94 taels SJC Vietnam gold",
          deliveryCoordination:
            "All financial institutions notified for physical gold transport",
        },
        brokerProfit: {
