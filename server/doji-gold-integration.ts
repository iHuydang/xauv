...
export class DojiGoldAPI extends EventEmitter {
    ...
    try {
      ...
      if (response.data) {
        ...
        if (data.SJC) {
          ...
          const sjcData = data.find(item => item.type === 'SJC' || item.name?.includes('SJC'));
          if (sjcData) {
        ...
        console.log(`🥇 Doji Gold - Buy: ${buyPrice.toLocaleString()} VND, Sell: ${sellPrice.toLocaleString()} VND`);
  ...
  async executeVietnamGoldPressure(intensity: 'LOW' | 'MEDIUM' | 'HIGH' = 'MEDIUM'): Promise<any> {
    try {
      console.log(`🚨 Executing Vietnam gold pressure attack - Intensity: ${intensity}`);
      ...
      const pressureVolume = intensity === 'HIGH' ? 1000000 : intensity === 'MEDIUM' ? 500000 : 250000;
      ...
      console.log(`💰 Estimated profit: ${attackResult.estimated_profit.toLocaleString()} VND`);
