...
export class InternationalPortfolioAccountManager extends EventEmitter {
  ...
  private setupPortfolioHoldings(): void {
    ...
    this.account.holdings.forEach((holding, symbol) => {
      console.log(`   📈 ${symbol}: ${holding.shares.toLocaleString()} shares, $${holding.marketValue.toLocaleString()}`);
    });
  }
  ...
  private updatePrices(): void {
    this.account.holdings.forEach(holding => {
      ...
      const priceChange = (Math.random() - 0.5) * volatility * holding.currentPrice;
      
      holding.currentPrice += priceChange;
      holding.marketValue = holding.shares * holding.currentPrice;
      holding.dailyChange = priceChange;
      holding.dailyChangePercent = (priceChange / (holding.currentPrice - priceChange)) * 100;
    });
  ...
  private generatePerformanceReport(): void {
    const totalGainLoss = Array.from(this.account.holdings.values())
      .reduce((sum, holding) => sum + holding.dailyChange * holding.shares, 0);
    ...
    console.log(`   Tổng giá trị: $${this.portfolioValue.toLocaleString()}`);
    console.log(`   Thay đổi hôm nay: ${totalGainLoss > 0 ? '+' : ''}$${totalGainLoss.toLocaleString()}`);
    console.log(`   Hiệu suất: ${performancePercent > 0 ? '+' : ''}${performancePercent.toFixed(2)}%`);
  ...
  public addHolding(holding: InternationalPortfolioHolding): void {
    this.account.holdings.set(holding.symbol, holding);
    this.calculatePortfolioValue();
    console.log(`✅ Đã thêm holding: ${holding.symbol} - ${holding.shares} shares`);
  }
  ...
  public getWorstPerformers(count: number = 3): InternationalPortfolioHolding[] {
    return Array.from(this.account.holdings.values())
...
export const internationalPortfolioAccountManager = new InternationalPortfolioAccountManager();
