...
export class LiquidityScannerAPI extends EventEmitter {
  ...
  async performSingleScan(side: 'buy' | 'sell' | 'both' = 'both'): Promise<ScanResult> {
  ...
  startMonitoring(side: 'buy' | 'sell' | 'both' = 'both', intervalSeconds: number = 15): string {
    const monitorId = `monitor_${Date.now()}_${Math.random().toString(36).substr(2, 8)}`;
    
    console.log(`Starting continuous monitoring: ${side} side, ${intervalSeconds}s interval`);
  ...
  async performDepthAnalysis(side: 'buy' | 'sell' = 'buy'): Promise<DepthAnalysis> {
    const currentPrice = 2680 + (Math.random() * 20 - 10);
    
    console.log(`Performing deep ${side} depth analysis at $${currentPrice.toFixed(2)}`);
    ...
    if (side === 'sell') {
      analysis.breakdownProbability = parseFloat((Math.random() * 100).toFixed(1));
    ...
    console.log(`Depth analysis completed: ${totalVolume.toLocaleString()} total volume`);
  ...
  getStatistics(): any {
    ...
    const buyScans = this.scanHistory.filter(s => s.side === 'buy' || s.side === 'both').length;
    const sellScans = this.scanHistory.filter(s => s.side === 'sell' || s.side === 'both').length;
    
    const avgBuyPressure = this.scanHistory
      .filter(s => s.liquidityData.buyPercent > 0)
      .reduce((sum, s) => sum + s.liquidityData.buyPercent, 0) / totalScans;
      
    const avgSellPressure = this.scanHistory
  ...
  async executeShellScanner(type: 'enhanced' | 'sell', command: 'single' | 'depth', side?: string): Promise<any> {
    try {
