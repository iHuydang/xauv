...
router.post("/sjc-bridge/simulate-trade", async (req, res) => {
  try {
    ...
    const simulatedTrade = {
      ...
      type,
      volume,
      openPrice: openPrice || (symbol === "XAUUSD" ? 2650.5 : 85000000),
      currentPrice: currentPrice || (symbol === "XAUUSD" ? 2651.0 : 85050000),
      profit: 0,
    };
    ...
    if (type === "buy") {
      simulatedTrade.profit =
        (simulatedTrade.currentPrice - simulatedTrade.openPrice) * volume * 100;
    } else {
      simulatedTrade.profit =
        (simulatedTrade.openPrice - simulatedTrade.currentPrice) * volume * 100;
    }

    const orderId =
      await sjcGoldBridge.convertExnessOrderToSJCGold(simulatedTrade);
  ...
  try {
    ...
    const metrics = providers.map((provider) => ({
      ...
      liquidityUtilization:
        ((provider.availableLiquidity / 50000000000) * 100).toFixed(2) + "%",
      goldInventoryTotal:
        provider.goldInventory.SJC_9999 +
        provider.goldInventory.SJC_COIN +
        provider.goldInventory.SJC_BAR,
    ...
    metrics.sort(
      (a, b) => parseFloat(b.performanceScore) - parseFloat(a.performanceScore),
    );
...
router.post("/sjc-bridge/emergency-liquidity", async (req, res) => {
  try {
    const { providerId, liquidityAmount, goldInventory } = req.body;
