...
router.post('/impact-check', async (req, res) => {
  try {
    const { symbols, timeframe, analysis_depth, include_correlations } = req.body;
...
router.get('/impact-monitor', async (req, res) => {
  try {
    ...
    const symbolArray = symbols.toString().split(',').map(s => s.trim());
...
// Helper functions
function calculateImpactScore(symbols: string[], timeframe: string = '1h'): number {
  let baseScore = 3.0;
  ...
  const majorCount = symbols.filter(s => majorPairs.includes(s.toUpperCase())).length;
  baseScore += majorCount * 1.2;
  
  // Gold/commodities have high impact
  const commodities = ['XAUUSD', 'XAGUSD', 'USOIL', 'UKOIL'];
  const commodityCount = symbols.filter(s => commodities.includes(s.toUpperCase())).length;
...
async function analyzeCorrelations(symbols: string[]): Promise<any> {
  ...
  correlations.neutral = symbols.filter(s => 
