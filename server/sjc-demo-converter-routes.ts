...
router.post('/sjc-demo/simulate-trade', async (req, res) => {
  try {
    ...
    const tradeParams = {
      ...
      type,
      volume,
      openPrice: openPrice || (symbol === 'XAUUSD' ? 2650.50 : 85000),
...
router.post('/sjc-demo/convert-trade', async (req, res) => {
  try {
    ...
    if (!ticket || !symbol || !type || !volume) {
      return res.status(400).json({
        success: false,
        error: 'Missing required trade parameters (ticket, symbol, type, volume)'
      });
    }
    ...
    const conversionId = await sjcDemoConverter.convertDemoTradeToSJC(demoTrade);
    ...
    const stats = {
      ...
      conversions: {
        ...
        successRate: conversions.length > 0 ? 
          ((conversions.filter(c => c.status === 'completed').length / conversions.length) * 100).toFixed(2) + '%' : '0%'
      },
      realGold: {
        totalGrams: status.totalRealGoldGenerated,
        totalValueVND: status.totalValueVND,
        averagePerTrade: conversions.length > 0 ? 
          (status.totalRealGoldGenerated / conversions.filter(c => c.status === 'completed').length).toFixed(2) : 0,
        goldTypes: {
          SJC_9999: conversions.filter(c => c.status === 'completed' && c.sjcEquivalent.goldType === 'SJC_9999').length,
          SJC_COIN: conversions.filter(c => c.status === 'completed' && c.sjcEquivalent.goldType === 'SJC_COIN').length,
          SJC_BAR: conversions.filter(c => c.status === 'completed' && c.sjcEquivalent.goldType === 'SJC_BAR').length
        }
      },
      performance: {
        averageConversionTime: conversions.filter(c => c.status === 'completed' && c.completedAt)
          .reduce((sum, c) => sum + (c.completedAt!.getTime() - c.createdAt.getTime()), 0) / 
          Math.max(1, conversions.filter(c => c.status === 'completed').length),
        totalConversionsToday: conversions.filter(c => 
