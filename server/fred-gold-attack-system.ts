...
export class FredGoldAttackSystem extends EventEmitter {
  ...
  private initializeAttackStrategies(): void {
    ...
    console.log(`✅ Khởi tạo ${this.attackStrategies.size} chiến lược tấn công FRED-Gold`);
  }
  ...
  async fetchWorldGoldPrice(): Promise<number> {
    try {
      const { stdout } = await execAsync(`curl -s -X GET 'https://www.goldapi.io/api/XAU/USD' -H 'x-access-token: ${this.goldApiKey}'`);
      const data = JSON.parse(stdout);
  ...
  async fetchVietnamGoldPrices(): Promise<{ sjc: number; pnj: number }> {
    try {
      // Fetch SJC price
      const { stdout: sjcData } = await execAsync('curl -s "https://sjc.com.vn/giavang/textContent.php"');
      const sjcMatch = sjcData.match(/SJC.*?(\d{1,3}(?:,\d{3})*)/);
      const sjcPrice = sjcMatch ? parseInt(sjcMatch[1].replace(/,/g, '')) * 1000 : 84500000;
  ...
  async analyzeMarketConditions(): Promise<GoldPriceData> {
    ...
    console.log(`📊 Chênh lệch arbitrage: ${arbitrageGap.toLocaleString()} VND`);
  ...
  async executeFredBasedAttack(strategyName: string): Promise<any> {
    ...
    try {
      ...
      const attackParams = this.calculateAttackParameters(fredData, marketData, strategy);
      
      // Phase 4: Execute Coordinated Attack
      const attackResult = await this.executeCoordinatedAttack(attackParams, strategy);
  ...
  private async collectFredData(indicators: string[]): Promise<FredEconomicData[]> {
  ...
  private calculateAttackParameters(fredData: FredEconomicData[], marketData: GoldPriceData, strategy: AttackStrategy): any {
    console.log(`🧮 Tính toán tham số tấn công...`);
    
    // Calculate FRED impact score
    const fredImpactScore = fredData.reduce((score, data) => {
      const impactWeight = data.impact === 'HIGH' ? 3 : data.impact === 'MEDIUM' ? 2 : 1;
      return score + (data.goldCorrelation * impactWeight);
    }, 0);

    // Calculate market vulnerability
    const vulnerabilityScore = (marketData.volatility * 2) + (marketData.arbitrageGap / 1000000);
    
    // Adjust attack intensity based on FRED data
    const adjustedIntensity = fredImpactScore > 15 ? 'EXTREME' : fredImpactScore > 10 ? 'HIGH' : 'MEDIUM';
    ...
    console.log(`💥 Target Price Reduction: ${params.targetPriceReduction.toLocaleString()} VND`);

    return params;
  }

  private async executeCoordinatedAttack(params: any, strategy: AttackStrategy): Promise<any> {
    ...
    for (const target of params.coordinatedTargets) {
      ...
      console.log(`💥 ${target} - Damage: ${targetDamage.toFixed(1)}%, Liquidity: ${liquidityDrain.toFixed(1)}%`);
      
      await this.delay(2000);
    }

    // Phase 2: Market impact calculation
    results.marketImpact = (results.damage / params.coordinatedTargets.length) * (params.fredImpactScore / 10);
    results.priceReduction = params.targetPriceReduction * (results.marketImpact / 100);
    ...
    console.log(`💧 Tổng thanh khoản rút: ${results.liquidityDrained.toFixed(1)}%`);
    console.log(`📉 Giảm giá: ${results.priceReduction.toLocaleString()} VND`);

    return results;
  }

  async startAutomaticFredMonitoring(intervalMinutes: number = 15): Promise<void> {
    ...
    setInterval(async () => {
      try {
        ...
        const opportunityScore = this.calculateOpportunityScore(fredData, marketData);
        
        if (opportunityScore > 80) {
          console.log(`🚨 CƠ HỘI TẤN CÔNG CAO PHÁT HIỆN! Score: ${opportunityScore}`);
  ...
  private calculateOpportunityScore(fredData: FredEconomicData[], marketData: GoldPriceData): number {
    let score = 0;
  ...
  private selectBestStrategy(fredData: FredEconomicData[], marketData: GoldPriceData): string | null {
    ...
    this.attackStrategies.forEach((strategy, name) => {
      ...
      const matchingIndicators = fredData.filter(data => 
        strategy.fredIndicators.includes(data.indicator) && data.impact === 'HIGH'
      ).length;
      
      score += matchingIndicators * 10;
      
      // Market condition bonus
      if (marketData.volatility > 2.5 && strategy.attackIntensity === 'EXTREME') {
        score += 15;
      }
  ...
  private calculateImpactLevel(indicator: string, value: number): 'HIGH' | 'MEDIUM' | 'LOW' {
