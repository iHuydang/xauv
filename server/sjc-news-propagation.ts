...
export class SJCNewsPropagationSystem extends EventEmitter {
  ...
  private async generateSellNews(order: any): Promise<void> {
    ...
    console.log(
      `⚖️ Khối lượng: ${fakeNews.goldTael.toFixed(2)} lượng (${fakeNews.goldWeight.toFixed(1)} gram)`,
    );
  ...
  private createFakeSellingNews(realOrder: any): SJCSellingNews {
    ...
    return {
      ...
      disguisedInfo: {
        sellerName:
          this.fakeSellerNames[
            Math.floor(Math.random() * this.fakeSellerNames.length)
          ],
        reason:
          this.sellReasons[Math.floor(Math.random() * this.sellReasons.length)],
  ...
  private formatNewsContent(news: SJCSellingNews): any {
    const urgencyText =
      news.disguisedInfo.urgency === "urgent" ? "[KHẨN CẤP] " : "";
    const impactText =
      news.marketImpact === "high"
        ? "🔴 "
        : news.marketImpact === "medium"
          ? "🟡 "
          : "🟢 ";
  ...
  private async sendToGoonus(newsContent: any, headers: any): Promise<void> {
    try {
      ...
      await axios.post(
        "https://goonus.io/api/gold-news/submit",
        goonusPayload,
        { headers },
      );
  ...
  private async sendToCafef(newsContent: any, headers: any): Promise<void> {
    try {
      ...
      await axios.post("https://cafef.vn/api/news/submit", cafefPayload, {
        headers,
      });
  ...
  private async sendToGenericSite(
    siteUrl: string,
    newsContent: any,
    headers: any,
  ): Promise<void> {
    try {
      ...
      for (const endpoint of endpoints) {
        try {
          await axios.post(siteUrl + endpoint, payload, {
            headers,
            timeout: 5000,
          });
  ...
  // Gửi qua backup API
  private async sendViaBackupAPI(
    siteUrl: string,
    newsContent: any,
  ): Promise<void> {
    try {
      ...
      await axios.post(
        "http://localhost:5000/api/news-propagation/backup",
        backupPayload,
      );
  ...
  private async sendViaWebhook(
    ...
    try {
      ...
      await axios.post(
        "http://localhost:5000/api/webhooks/news-distribution",
        webhookPayload,
      );
  ...
  private createMarketPressure(news: SJCSellingNews): void {
    ...
    console.log(
      `📉 Tín hiệu: Có người bán ${news.goldTael.toFixed(1)} lượng với giá thấp`,
    );
