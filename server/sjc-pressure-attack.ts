...
export class SJCPressureAttack extends EventEmitter {
  ...
  private evaluateAutoAttack(opportunity: any): void {
    ...
    if (attackIntensity === "CỰC MẠNH" || attackIntensity === "MẠNH") {
      const vectorName = this.selectOptimalVector(
        vulnerabilities,
        targetSpread,
      );
      if (vectorName) {
  ...
  private selectOptimalVector(
    ...
    this.attackVectors.forEach((vector, name) => {
      ...
      if (
        vulnerabilities.includes("SPREAD_CAO") &&
        vector.targetSpread <= targetSpread
      ) {
        score += 20;
      }

      if (
        vulnerabilities.includes("THANH_KHOẢN_THẤP") &&
        vector.intensity === "EXTREME"
      ) {
        score += 15;
      }

      if (
        vulnerabilities.includes("PREMIUM_CAO") &&
        name === "PREMIUM_EXPLOIT"
      ) {
        score += 25;
      }
    ...
    console.log(
      `🎯 Selected optimal vector: ${bestVector} (score: ${bestScore})`,
    );
    return bestVector;
  }
  ...
  async executeAttack(
    vectorName: string,
    options: { autoTriggered?: boolean; sourceIP?: string } = {},
  ): Promise<string> {
    ...
    console.log(
      `🎯 Target Spread: ${vector.targetSpread.toLocaleString()} VND`,
    );
    if (options.sourceIP) {
  ...
  private async executeReconnaissance(attackId: string): Promise<void> {
    ...
    if (sjcData) {
      console.log(
        `📊 SJC Current Spread: ${sjcData.spread.toLocaleString()} VND`,
      );
      console.log(`💰 SJC Buy Price: ${sjcData.buyPrice.toLocaleString()} VND`);
      console.log(
        `🎯 Liquidity Level: ${sjcData.liquidityLevel.toUpperCase()}`,
      );
    }
  ...
  private async executeInitialPressure(
    attackId: string,
    vector: AttackVector,
  ): Promise<void> {
    ...
    console.log(
      `📈 Initial damage: ${attack.damageInflicted.spreadReduction.toFixed(0)} VND spread reduction`,
    );
  }

  private async executeMainAttack(
    attackId: string,
    vector: AttackVector,
  ): Promise<void> {
    ...
    while (Date.now() - startTime < attackDuration) {
      ...
      if ((Date.now() - startTime) % 10000 < 1000) {
        const progress = (
          ((Date.now() - startTime) / attackDuration) *
          100
        ).toFixed(1);
        console.log(
          `⚔️ Attack progress: ${progress}% - Damage: ${attack.damageInflicted.spreadReduction.toFixed(0)} VND`,
        );
      }
  ...
  private calculateAttackDamage(vector: AttackVector): any {
    const baseSpreadReduction =
      vector.targetSpread * 0.05 * vector.volumeMultiplier;
  ...
  private async executeDamageAssessment(attackId: string): Promise<void> {
    ...
    if (postAttackData) {
      ...
      console.log(
        `📉 Total Spread Reduction: ${attack.damageInflicted.spreadReduction.toFixed(0)} VND`,
      );
      console.log(
        `💧 Liquidity Drained: ${attack.damageInflicted.liquidityDrained.toFixed(1)}%`,
      );
      console.log(
        `📊 Market Share Impact: ${attack.damageInflicted.marketShare.toFixed(1)}%`,
      );
      console.log(
        `🛡️ SJC Defense Responses: ${attack.sjcResponse.priceAdjustments}`,
      );
      console.log(
        `📊 Final SJC Spread: ${postAttackData.spread.toLocaleString()} VND`,
      );
    }
  }
  ...
  private analyzeSJCVulnerabilities(sjcData: any): void {
    ...
    if (spreadRatio > 0.015) {
      console.log(
        `⚠️ SJC vulnerability detected: High spread ratio ${(spreadRatio * 100).toFixed(2)}%`,
      );
