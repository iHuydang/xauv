...
router.get('/api/xtb/prices', async (req, res) => {
  try {
    ...
    const symbolList = symbols ? symbols.toString().split(',') : ['XAUUSD', 'EURUSD', 'GBPUSD'];
  ...
  try {
    ...
    if (range_min && range_max) {
      ...
      liquidityAnalysis['rangeAnalysis'] = {
        ...
        distanceFromMin: ((currentPrice - minPrice) / minPrice * 100).toFixed(2) + '%',
        distanceFromMax: ((maxPrice - currentPrice) / maxPrice * 100).toFixed(2) + '%',
        positionInRange: ((currentPrice - minPrice) / (maxPrice - minPrice) * 100).toFixed(1) + '%'
      };
    }
