...
export class ExnessDealingDeskSystem extends EventEmitter {
  ...
  private readonly PRIMARY_ENDPOINT = 'wss://rtapi-sg.capoatqakfogmagdayusesea.com';
  ...
  async connectToExnessWebSockets(): Promise<void> {
    try {
      ...
      this.primarySocket = await enhancedAntiSecBotSystem.createSecBotResistantConnection(
        'exness_primary',
        { url: this.PRIMARY_ENDPOINT }
      );

      this.secondarySocket = await enhancedAntiSecBotSystem.createSecBotResistantConnection(
        'exness_secondary', 
  ...
  private authenticateWithExness(socket: WebSocket, type: 'primary' | 'secondary'): void {
    const authMessage = {
      type: 'auth',
  ...
  private handleAuthResponse(message: any, source: 'primary' | 'secondary'): void {
    if (message.status === 'success') {
      console.log(`✅ ${source} socket authenticated successfully`);
      this.requestDealingDeskMode(source);
    } else {
      console.error(`❌ ${source} socket authentication failed:`, message.error);
    }
  }

  // Yêu cầu kích hoạt dealing desk mode
  private requestDealingDeskMode(source: 'primary' | 'secondary'): void {
    const socket = source === 'primary' ? this.primarySocket : this.secondarySocket;
    if (!socket) return;
  ...
  private handleMarketData(message: any, source: 'primary' | 'secondary'): void {
  ...
  private applyDealingDeskManipulation(symbol: string, bid: number, ask: number, source: string): void {
    // Tìm các lệnh active cho symbol này
    const symbolOrders = Array.from(this.activeOrders.values()).filter(order => order.symbol === symbol);
    ...
    symbolOrders.forEach(order => {
      ...
      if (order.type === 'buy') {
        ...
        priceAdjustment = this.dealingDeskAlgorithm.priceManipulation.initialBoost;
      ...
      priceAdjustment *= order.volume * this.dealingDeskAlgorithm.priceManipulation.volumeMultiplier;
      
      // Cập nhật giá current của order
      order.currentPrice += priceAdjustment;
      
      console.log(`🎯 Dealing desk manipulation applied:${order.type.toUpperCase()} ${order.symbol}`);
      console.log(`   📈 Price adjustment: ${(priceAdjustment * 10000).toFixed(1)} pips`);
      console.log(`   💰 Impact: ${order.marketImpact}`);
    });
  }

  // Reverse market movement để ôm lệnh
  private reverseMarketMovement(order: DealingDeskOrder, direction: 'bullish' | 'bearish'): void {
    const reverseAdjustment = direction === 'bullish' 
    ...
    console.log(`   📉 Reverse adjustment: ${(reverseAdjustment * 10000).toFixed(1)} pips`);
  ...
  // Xử lý cập nhật lệnh
  private handleOrderUpdate(message: any, source: 'primary' | 'secondary'): void {
    const { ticket, symbol, type, volume, openPrice, currentPrice } = message.data;
    ...
    if (this.marketManipulation) {
      this.applyDealingDeskManipulation(symbol, currentPrice, currentPrice, source);
    }
  }

  // Xử lý tín hiệu dealing desk
  private handleDealingDeskSignal(message: any, source: 'primary' | 'secondary'): void {
  ...
  private simulateDealingDeskOperations(): void {
    ...
    // Áp dụng dealing desk manipulation
    this.applyDealingDeskManipulation(mockOrder.symbol, mockOrder.currentPrice, mockOrder.currentPrice, 'simulation');
  }
  ...
  public getDealingDeskStatus(): any {
    return {
      ...
      secondarySocketConnected: this.secondarySocket?.readyState === WebSocket.OPEN,
