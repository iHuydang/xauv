...
router.post("/vietnam-gold/quick-attack/:target", async (req, res) => {
  try {
    ...
    const attackId = await vietnamGoldBroker.executeQuickPressureAttack(
      [target],
      intensity,
    );
...
router.post("/vietnam-gold/devastation-attack", async (req, res) => {
  try {
    ...
    const attackId =
      await vietnamGoldBroker.executePressureAttack(devastationConfig);
  ...
  try {
    ...
    const analysis = prices.map((price) => ({
      ...
      attack_recommendation:
        price.vulnerability_score > 7
          ? "IMMEDIATE_ATTACK"
          : price.vulnerability_score > 5
            ? "PREPARE_ATTACK"
            : "MONITOR",
      optimal_time:
        price.vulnerability_score > 5 ? "NOW" : "WAIT_FOR_OPPORTUNITY",
    }));
    ...
    res.json({
      ...
      data: {
        ...
        attack_ready_count: analysis.filter((a) => a.vulnerability_score > 7)
          .length,
        timestamp: Date.now(),
      },
    });
