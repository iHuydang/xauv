...
export class DemoToRealLiquidityConverter extends EventEmitter {
  ...
  private initializeLiquidityConverter(): void {
    ...
    console.log(
      `💰 SJC Broker Balance: ${this.ecnEngine.sjcBrokerBalance.toLocaleString()} VND`,
    );
    console.log(
      `📊 ECN Efficiency: ${(this.ecnEngine.ecnEfficiencyRating * 100).toFixed(1)}%`,
    );
  ...
  public registerDemoOrder(order: DemoOrder): void {
    ...
    console.log(
      `   Symbol: ${order.symbol} | Side: ${order.side.toUpperCase()}`,
    );
    console.log(
      `   Volume: ${order.volume} | Profit: ${order.profit.toLocaleString()} VND`,
    );
  ...
  private scheduleConversion(order: DemoOrder): void {
    ...
    console.log(
      `🥇 Real SJC: ${sjcVolumeGram.toFixed(2)} gram (${realConversionAmount.toLocaleString()} VND)`,
    );
    console.log(`🏦 LP Account: ${conversion.liquidityProviderAccount}`);
    console.log(
      `📊 Market Impact: ${conversion.marketImpactBps.toFixed(1)} bps`,
    );
  ...
  private processEligibleDemoOrders(): void {
    const eligibleOrders = Array.from(this.demoOrders.values()).filter(
      (order) => {
        const profitVND = Math.abs(order.profit);
        return (
          profitVND >= this.conversionRates.minProfitThreshold &&
  ...
  private async executeRealLiquidityProvision(): Promise<void> {
    const pendingConversions = Array.from(
      this.realLiquidityImpacts.values(),
    ...
    for (const conversion of pendingConversions) {
      try {
        ...
        console.error(
          `❌ Conversion failed: ${conversion.conversionId}`,
          error,
        );
      }
    }
  ...
  private async executeConversion(
    conversion: RealLiquidityImpact,
  ): Promise<void> {
    ...
    if (this.ecnEngine.sjcBrokerBalance < conversion.realSJCValue) {
      console.log(
        `⚠️ Insufficient SJC broker balance for ${conversion.conversionId}`,
      );
      return;
    }
    ...
    this.ecnEngine.totalProfitConverted += Math.abs(
      conversion.originalDemoOrder.profit,
    );
    ...
    console.log(
      `🥇 Provided ${conversion.realSJCVolume.toFixed(2)}g SJC liquidity`,
    );
    console.log(`💰 Value: ${conversion.realSJCValue.toLocaleString()} VND`);
    console.log(
      `📊 Remaining Balance: ${this.ecnEngine.sjcBrokerBalance.toLocaleString()} VND`,
    );
  ...
  private async applyECNMarketImpact(
    conversion: RealLiquidityImpact,
  ): Promise<void> {
    ...
    if (currentGoldPrice) {
      ...
      console.log(
        `   Original: ${currentGoldPrice.bid}/${currentGoldPrice.ask}`,
      );
      console.log(
        `   Adjusted: ${adjustedBid.toFixed(5)}/${adjustedAsk.toFixed(5)}`,
      );
  ...
  private settlePendingConversions(): void {
    const executedConversions = Array.from(
      this.realLiquidityImpacts.values(),
  ...
  private isOrderAlreadyConverted(orderId: string): boolean {
    return Array.from(this.realLiquidityImpacts.values()).some(
      (conv) => conv.originalDemoOrder.orderId === orderId,
    );
  }

  private updateECNMetrics(): void {
    const totalConversions = this.realLiquidityImpacts.size;
    const successfulConversions = Array.from(
      this.realLiquidityImpacts.values(),
    ).filter((conv) => conv.status === "settled").length;

    if (totalConversions > 0) {
      this.ecnEngine.ecnEfficiencyRating =
  ...
  public async convertDemoProfit(
    ...
    if (
      !forceConversion &&
      Math.abs(order.profit) < this.conversionRates.minProfitThreshold
    ) {
    ...
    const conversions = Array.from(this.realLiquidityImpacts.values()).filter(
      (conv) => conv.originalDemoOrder.orderId === orderId,
    );
  ...
  public updateConversionRates(
    rates: Partial<typeof this.conversionRates>,
  ): void {
  ...
  public async injectEmergencyLiquidity(amountVND: number): Promise<void> {
    ...
    console.log(
      `🚨 Emergency liquidity injected: ${amountVND.toLocaleString()} VND`,
    );
    console.log(
      `💰 New balance: ${this.ecnEngine.sjcBrokerBalance.toLocaleString()} VND`,
    );
