  ...
  try {
    ...
    if (type === "buy") {
      ...
      setTimeout(() => {
        ...
        console.log(
          `🔄 Reverse movement applied to order ${mockOrder.ticket}: -5 pips`,
        );
      }, 30000);
    } else {
    ...
    mockOrder.profit =
      (mockOrder.currentPrice - mockOrder.openPrice) * volume * 100000;

    console.log(`🎯 Dealing desk simulation:`);
    console.log(`   Order: ${type.toUpperCase()} ${volume} ${symbol}`);
    console.log(
      `   Initial manipulation: ${(priceAdjustment * 10000).toFixed(1)} pips`,
    );
  ...
  try {
    ...
    if (!orderType || !marketDirection) {
      return res.status(400).json({
        success: false,
        error:
          "Missing parameters: orderType (buy/sell), marketDirection (up/down)",
      });
    }
...
router.post("/exness/convert-account", async (req, res) => {
  try {
    ...
    console.log(
      `   Dealing desk: ${enableDealingDesk ? "ENABLED" : "DISABLED"}`,
    );
