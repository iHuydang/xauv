...
export class SignalProcessor {
  ...
  private generateFEDSignals(): MarketSignal[] {
    ...
    if (Math.random() > 0.95) { // 5% chance per check
      signals.push({
        ...
        impact: ['high', 'very_high', 'extreme'][Math.floor(Math.random() * 3)] as any,
        direction: Math.random() > 0.5 ? 'bullish' : 'bearish',
        confidence: Math.random() * 30 + 70, // 70-100% confidence
        symbols: ['EURUSD', 'GBPUSD', 'USDJPY', 'USDCAD', 'AUDUSD'],
        timestamp: new Date(),
        metadata: {
          eventType: ['FOMC_MEETING', 'RATE_DECISION', 'POWELL_SPEECH'][Math.floor(Math.random() * 3)],
  ...
  private generateInstitutionalSignals(): MarketSignal[] {
    ...
    if (Math.random() > 0.92) { // 8% chance per check
      const institutions = [
        'Goldman Sachs', 'JP Morgan', 'Citadel', 'Bridgewater',
      ...
      signals.push({
        ...
        impact: ['medium', 'high', 'very_high'][Math.floor(Math.random() * 3)] as any,
        direction: Math.random() > 0.5 ? 'bullish' : 'bearish',
        confidence: Math.random() * 25 + 75, // 75-100% confidence
        symbols: ['EURUSD', 'GBPUSD', 'USDJPY', 'XAUUSD'][Math.floor(Math.random() * 4)],
        timestamp: new Date(),
        metadata: {
          orderSize: Math.random() * 10000000 + 1000000, // 1M - 10M USD
          timeframe: ['scalp', 'intraday', 'swing'][Math.floor(Math.random() * 3)]
        }
      });
    }
  ...
  private generateBrokerSignals(): MarketSignal[] {
    ...
    if (Math.random() > 0.88) { // 12% chance per check
      const brokers = [
        'Exness', 'IC Markets', 'Pepperstone', 'FP Markets',
        'XM', 'OANDA', 'Interactive Brokers'
      ];
  ...
  private generateCentralBankSignals(): MarketSignal[] {
    ...
    if (Math.random() > 0.97) { // 3% chance per check
      const centralBanks = [
    ...
    for (const accountId of targetAccounts) {
      for (const symbol of signal.symbols) {
        ...
        setTimeout(() => {
          ...
          console.log(`✅ Order executed: ${symbol} for account ${accountId}`);
        }, Math.random() * 2000 + 500); // 0.5-2.5 second execution delay
  ...
  private calculateOptimalVolume(signal: MarketSignal, symbol: string): number {
    ...
    const volume = baseVolume * impactMultiplier[signal.impact] * confidenceMultiplier;
