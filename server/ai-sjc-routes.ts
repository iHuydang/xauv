...
router.get("/exploitation-score", async (req, res) => {
  try {
    ...
    res.json({
      ...
      ai_assessment:
        analysis.exploitation_score > 80
          ? "EXTREME_EXPLOITATION"
          : analysis.exploitation_score > 60
            ? "HIGH_EXPLOITATION"
            : analysis.exploitation_score > 40
...
router.post("/simulate-disruption", async (req, res) => {
  try {
    ...
    const simulation =
      await aiSJCMonopolyBreaker.simulateMarketDisruption(strategy);
...
router.post("/optimal-timing", async (req, res) => {
  try {
    const timing = await (
      aiSJCMonopolyBreaker as any
    ).calculateOptimalDisruptionTiming();
...
router.post("/execute-monopoly-breaking", async (req, res) => {
  try {
    ...
    console.log(
      `🤖 AI executing monopoly breaking with intensity ${intensity}`,
    );

    const result =
      await aiSJCMonopolyBreaker.executeMonopolyBreaking(intensity);
...
router.post("/auto-sentiment-generation", async (req, res) => {
  try {
    ...
    console.log(
      `🎭 AUTO GENERATING SENTIMENT POST ${post_number}/${total_posts}`,
    );
    ...
    const contentTemplates = [
      {
        ...
        content:
          "Phân tích cho thấy độc quyền SJC làm tăng giá vàng 20-30% so với thế giới, gây thiệt hại hàng nghìn tỷ đồng mỗi năm cho người dân.",
      },
      {
        title: "Hướng dẫn mua vàng quốc tế giá rẻ, tránh bẫy SJC",
        content:
          "PAXG, XAUT - các token vàng quốc tế giúp bạn sở hữu vàng với giá thật, không bị SJC thổi giá.",
      },
      {
        title: "So sánh: Vàng SJC vs Vàng thế giới - Chênh lệch khủng khiếp",
        content:
          "Trong khi vàng thế giới $2650/oz, SJC bán 80 triệu/lượng. Chênh lệch 15-20 triệu/lượng = 40% lợi nhuận độc quyền!",
      },
      {
        title: "Kinh tế gia cảnh báo: Độc quyền SJC cản trở phát triển kinh tế",
        content:
          "Các chuyên gia kinh tế quốc tế khuyến cáo Việt Nam cần chấm dứt độc quyền vàng để phát triển thị trường tài chính minh bạch.",
      },
      {
        title: "Người dân thức tỉnh: Tẩy chay SJC, chọn vàng quốc tế",
        content:
          'Phong trào tẩy chay SJC đang lan rộng. Nhiều người chuyển sang mua vàng quốc tế qua các sàn uy tín để tránh bị "chặt chém".',
      },
    ];

    const selectedTemplate =
      contentTemplates[content_variation - 1] || contentTemplates[0];
    const intensityLevels = ["subtle", "moderate", "aggressive", "extreme"];
    const selectedIntensity =
      intensityLevels[intensity_variation - 1] || "moderate";
    ...
    const batchResult = {
      ...
      scheduled_time: new Date(
        Date.now() + (post_number - 1) * time_interval_minutes * 60000,
      ).toISOString(),
...
router.get("/status", async (req, res) => {
  try {
    ...
    res.json({
      ...
      swarm_agents: aiSJCMonopolyBreaker
        .getSwarmStatus()
  ...
  try {
    ...
    res.json({
      ...
      overall_effectiveness:
        Object.values(metricsObject).reduce((sum, val) => sum + val, 0) /
        Object.keys(metricsObject).length,
