...
router.post('/ecn/register-demo-order', async (req, res) => {
  try {
    const { orderId, symbol, side, volume, openPrice, currentPrice, profit, mt5Account } = req.body;
    ...
    res.status(500).json({
      success: false,
      error: error instanceof Error ? error.message : 'Failed to register demo order'
    });
  }
});
...
router.post('/ecn/update-rates', async (req, res) => {
  try {
    ...
    const rates: any = {};
    if (demoToRealMultiplier !== undefined) rates.demoToRealMultiplier = parseFloat(demoToRealMultiplier);
    if (minProfitThreshold !== undefined) rates.minProfitThreshold = parseInt(minProfitThreshold);
    if (maxSingleConversion !== undefined) rates.maxSingleConversion = parseInt(maxSingleConversion);
    if (ecnSpreadImpact !== undefined) rates.ecnSpreadImpact = parseFloat(ecnSpreadImpact);
...
router.post('/ecn/inject-liquidity', async (req, res) => {
  try {
    ...
    await demoToRealLiquidityConverter.injectEmergencyLiquidity(parseInt(amount));
...
router.post('/ecn/simulate-demo-order', async (req, res) => {
  try {
    ...
    if (mockOrder.profit > 50000) {
      try {
        const conversionId = await demoToRealLiquidityConverter.convertDemoProfit(mockOrder.orderId);
        conversionResult = { conversionId, status: 'conversion_triggered' };
      } catch (error) {
        conversionResult = { error: error instanceof Error ? error.message : 'Conversion failed' };
      }
    }
