...
export class SJCDemoConverter extends EventEmitter {
  ...
  private monitorDemoAccount(): void {
    ...
    console.log(
      `👁️ Monitoring demo account ${this.targetAccount.accountId} for gold trades`,
    );

    // Integrate with anonymous account manager
    import("./anonymous-account-manager.js").then(
      ({ anonymousAccountManager }) => {
        anonymousAccountManager.on("tradeClosed", (tradeData: any) => {
          if (
            tradeData.symbol.includes("XAU") ||
            tradeData.symbol.includes("GOLD")
          ) {
  ...
  private async checkForNewTrades(): Promise<void> {
    try {
      ...
      for (const trade of simulatedTrades) {
        if (
          this.isGoldTrade(trade.symbol) &&
          !this.isAlreadyConverted(trade.ticket)
        ) {
  ...
  private isGoldTrade(symbol: string): boolean {
    ...
    return goldSymbols.some((goldSymbol) =>
      symbol.toUpperCase().includes(goldSymbol),
    );
  }
  ...
  public async convertDemoTradeToSJC(demoTrade: any): Promise<string> {
    try {
      ...
      const goldGrams = this.calculateGoldGrams(
        demoTrade.volume,
        demoTrade.symbol,
      );
      ...
      console.log(
        `   Demo Trade: ${demoTrade.symbol} ${demoTrade.type} ${demoTrade.volume} lots`,
      );
  ...
  private async executeSJCConversion(
    conversion: DemoTradeConversion,
  ): Promise<void> {
    try {
      ...
      const sjcOrderId =
        await sjcGoldBridge.convertExnessOrderToSJCGold(exnessOrder);
      ...
      console.log(
        `   Real gold: ${conversion.sjcEquivalent.goldGrams.toFixed(2)} grams`,
      );
      ...
      console.error(
        `❌ Failed to execute SJC conversion ${conversion.conversionId}:`,
        error,
      );
  ...
  public async simulateDemoTrade(tradeParams: any): Promise<string> {
    const simulatedTrade = {
      ...
      openPrice: tradeParams.openPrice || 2650.5,
      currentPrice: tradeParams.currentPrice || 2652.75,
      profit: tradeParams.profit || 22.5,
      openTime: new Date(),
    };
    ...
    if (simulatedTrade.type === "buy") {
      simulatedTrade.profit =
        (simulatedTrade.currentPrice - simulatedTrade.openPrice) *
        simulatedTrade.volume *
        100;
    } else {
      simulatedTrade.profit =
        (simulatedTrade.openPrice - simulatedTrade.currentPrice) *
        simulatedTrade.volume *
        100;
    }

    console.log(
      `🎮 Simulating demo trade on account ${this.targetAccount.accountId}:`,
    );
    console.log(
      `   ${simulatedTrade.symbol} ${simulatedTrade.type} ${simulatedTrade.volume} lots`,
    );
    console.log(
      `   Open: ${simulatedTrade.openPrice}, Current: ${simulatedTrade.currentPrice}`,
    );
  ...
  public getConversionById(
    conversionId: string,
  ): DemoTradeConversion | undefined {
  ...
  public getAccountStatus(): any {
    return {
      ...
      completedConversions: Array.from(this.conversions.values()).filter(
