...
export class StableConnectionManager extends EventEmitter {
  ...
  registerConnection(
    ...
    if (this.isShuttingDown) {
      console.log(`⚠️  Skipping connection registration for ${id} - system shutting down`);
      return;
    }
  ...
  private handleConnectionFailure(id: string): void {
    ...
    console.log(`🔄 Retry attempt ${attempts + 1}/${this.config.maxRetries} for ${id}`);
    ...
    }, this.config.retryDelay * (attempts + 1)); // Exponential backoff
  }
