...
export class VietnamGoldBroker extends EventEmitter {
  ...
  private async scanAllVietnameseGoldSources(): Promise<void> {
    ...
    try {
      ...
      console.log(
        `📊 Đã quét ${this.goldPrices.size} nguồn giá vàng qua broker`,
      );
    } catch (error) {
  ...
  private async scanDOJIPrice(): Promise<void> {
    try {
      ...
      const response = await axios.get(
        `${this.apiConfigs.doji.baseUrl}/?api_key=${this.apiConfigs.doji.apiKey}`,
        {
          headers: this.apiConfigs.doji.headers,
          timeout: 10000,
        },
      ...
      if (response.data && response.data.rows) {
        const sjcData = response.data.rows.find(
          (row: any) => row.type && row.type.toLowerCase().includes("sjc"),
        );

        if (sjcData) {
          const buyPrice =
            parseFloat(sjcData.buy?.replace(/[^\d.]/g, "") || "0") * 1000;
          const sellPrice =
            parseFloat(sjcData.sell?.replace(/[^\d.]/g, "") || "0") * 1000;
          ...
          if (buyPrice > 0 && sellPrice > 0) {
            ...
            console.log(
              `💰 DOJI: Mua ${buyPrice.toLocaleString()}, Bán ${sellPrice.toLocaleString()}`,
            );
          }
        }
      }
  ...
  private async scanSJCPrice(): Promise<void> {
    try {
      ...
      const response = await axios.get(
        this.apiConfigs.sjc.baseUrl + this.apiConfigs.sjc.endpoint,
        {
          headers: {
            "User-Agent":
              "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36",
      ...
      if (sjcMatch) {
        ...
        if (buyPrice > 0 && sellPrice > 0) {
          ...
          console.log(
            `🥇 SJC: Mua ${buyPrice.toLocaleString()}, Bán ${sellPrice.toLocaleString()}`,
          );
        }
      }
    } catch (error) {
  ...
  private async scanPNJPrice(): Promise<void> {
    try {
      ...
      const response = await axios.post(
        this.apiConfigs.pnj.baseUrl + this.apiConfigs.pnj.endpoint,
        payload,
        {
          headers: {
      ...
      if (
        ...
        if (buyPrice > 0) {
          ...
          console.log(
            `💎 PNJ: Mua ${buyPrice.toLocaleString()}, Bán ${sellPrice.toLocaleString()}`,
          );
        }
      }
    } catch (error) {
  ...
  private async scanTyGiaPrice(): Promise<void> {
    try {
      ...
      const params = new URLSearchParams(
        this.apiConfigs.tygia.params as any,
      ).toString();
      const response = await axios.get(
        `${this.apiConfigs.tygia.baseUrl}${this.apiConfigs.tygia.endpoints.json}?${params}`,
        {
          headers: {
            "User-Agent":
              "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36",
      ...
      if (response.data && response.data.gold) {
        ...
        const sjcData = goldData.find(
          (item: any) => item.name && item.name.toLowerCase().includes("sjc"),
        );
        ...
        if (sjcData) {
          ...
          if (buyPrice > 0 && sellPrice > 0) {
            ...
            console.log(
              `📈 TyGia SJC: Mua ${buyPrice.toLocaleString()}, Bán ${sellPrice.toLocaleString()}`,
            );
          }
        }
      }
  ...
  private analyzeVulnerabilities(): void {
    ...
    const highVulnerabilityTargets = sources.filter(
      (source) => source.vulnerability_score > 5.0,
    );
    ...
    if (highVulnerabilityTargets.length > 0) {
      ...
      highVulnerabilityTargets.forEach((target) => {
        console.log(
          `   ${target.source}: Điểm yếu ${target.vulnerability_score.toFixed(1)}/10`,
        );
      });
  ...
  private detectArbitrageOpportunities(sources: VietnamGoldPrice[]): void {
    ...
    for (let i = 0; i < sources.length; i++) {
      for (let j = i + 1; j < sources.length; j++) {
        ...
        const arbitragePercent =
          (arbitrage / Math.min(source1.buy, source2.buy)) * 100;
        ...
        if (arbitrage > maxArbitrage && arbitragePercent > 0.5) {
          ...
          bestOpportunity = {
            ...
            arbitragePercent,
            buyFrom:
              source1.buy < source2.buy ? source1.source : source2.source,
    ...
    if (bestOpportunity && maxArbitrage > 50000) {
      ...
      console.log(
        `💰 Lợi nhuận: ${maxArbitrage.toLocaleString()} VND (${bestOpportunity.arbitragePercent.toFixed(2)}%)`,
      );
  ...
  private async executeBrokerPressureIfNeeded(): Promise<void> {
    const sources = Array.from(this.goldPrices.values());
    const highVulnerabilityTargets = sources.filter(
      (source) => source.vulnerability_score > 7.0,
    );
  ...
  public async executePressureAttack(
    config: PressureAttackConfig,
  ): Promise<string> {
  ...
  private async executeBrokerReconnaissance(
    config: PressureAttackConfig,
  ): Promise<void> {
  ...
  private async executeCoordinatedPressureOrders(
    config: PressureAttackConfig,
  ): Promise<void> {
  ...
  private async executeMarketAbsorption(
    config: PressureAttackConfig,
  ): Promise<void> {
    console.log("🌊 Phase 3: Hấp thụ thanh khoản thị trường...");

    // Execute reverse orders to absorb market movement
    const absorptionOrders = Math.floor(
      3 * this.getIntensityMultiplier(config.intensity),
    );

    for (let i = 0; i < absorptionOrders; i++) {
      const target =
        config.target_sources[
          Math.floor(Math.random() * config.target_sources.length)
        ];
      const brokerAccount =
        this.brokerAccounts[
          Math.floor(Math.random() * this.brokerAccounts.length)
        ];
  ...
  private async executeBrokerOrder(order: BrokerOrder): Promise<void> {
    ...
    console.log(
      `📝 Broker Order: ${order.side.toUpperCase()} ${order.volume}kg ${order.target} @ ${order.price.toLocaleString()} (${order.broker_account})`,
    );
  ...
  private applyMarketImpact(order: BrokerOrder): void {
    ...
    if (order.side === "buy") {
      ...
      console.log(
        `📈 ${order.target} tăng ${impactAmount.toLocaleString()} VND do áp lực mua`,
      );
    } else {
      ...
      priceData.sell -= impactAmount;
      console.log(
        `📉 ${order.target} giảm ${impactAmount.toLocaleString()} VND do áp lực bán`,
      );
    }

    // Update vulnerability score
    priceData.vulnerability_score = this.calculateVulnerabilityScore(
      priceData.spread,
      priceData.buy,
    );
  }
  ...
  private analyzeMarketDepth(): number {
    ...
    const avgSpread =
      sources.reduce((sum, s) => sum + s.spread, 0) / sources.length;
    const avgPrice =
      sources.reduce((sum, s) => sum + s.buy, 0) / sources.length;
  ...
  private calculateAttackEffectiveness(): number {
    ...
    const recentOrders = Array.from(this.activeBrokerOrders.values()).filter(
      (order) => Date.now() - order.execution_time < 300000,
    ); // Last 5 minutes
  ...
  public async executeQuickPressureAttack(
    targets: string[],
