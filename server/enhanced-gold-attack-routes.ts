  ...
  try {
    ...
    res.json({
      ...
      data: {
        ...
        most_effective: vectors.sort((a, b) => b.effectiveness_rate - a.effectiveness_rate).slice(0, 3),
        recommended_fsapi: vectors.filter(v => v.target === 'FSAPI' || v.target === 'ALL')
      }
    });
  } catch (error) {
    res.status(500).json({
...
router.get('/gold-attack/market-data', async (req, res) => {
  try {
    ...
    const dataArray = Array.from(marketData.entries()).map(([source, data]) => ({
      source,
      ...data
    }));
    ...
    res.json({
      ...
      data: {
        ...
        highest_vulnerability: dataArray.sort((a, b) => (b.vulnerability_score || 0) - (a.vulnerability_score || 0)).slice(0, 3),
        best_targets: dataArray.filter(d => (d.vulnerability_score || 0) > 60)
      }
    });
  } catch (error) {
    res.status(500).json({
...
router.post('/gold-attack/execute', async (req, res) => {
  try {
    ...
    const result = await enhancedGoldAttackSystem.executeEnhancedAttack(vector_name, custom_params);
...
router.post('/gold-attack/sjc-pressure', async (req, res) => {
  try {
    const { intensity = 'EXTREME', duration_minutes = 30, volume_multiplier = 8.5 } = req.body;
    ...
    const result = await enhancedGoldAttackSystem.executeEnhancedAttack('SJC_PRESSURE_EXTREME', customParams);
...
router.post('/gold-attack/fsapi-drain', async (req, res) => {
  try {
    ...
    const result = await enhancedGoldAttackSystem.executeEnhancedAttack('FSAPI_LIQUIDITY_DRAIN', customParams);
    ...
    res.json({
      ...
      data: {
        ...
        parameters: customParams,
        result,
        message: 'FSAPI liquidity drain attack - highest effectiveness against fsapi.gold.org'
      }
    });
  } catch (error) {
    res.status(500).json({
...
router.post('/gold-attack/coordinated', async (req, res) => {
  try {
    const { intensity = 'MAXIMUM', duration_minutes = 90, volume_multiplier = 20.0 } = req.body;
    ...
    const result = await enhancedGoldAttackSystem.executeEnhancedAttack('MULTI_TARGET_COORDINATED', customParams);
...
router.post('/gold-attack/world-arbitrage', async (req, res) => {
  try {
    ...
    const result = await enhancedGoldAttackSystem.executeEnhancedAttack('WORLD_ARBITRAGE_COORDINATED', customParams);
    ...
    res.json({
      ...
      data: {
        ...
        parameters: customParams,
        result,
        message: 'World gold arbitrage attack using international price differences'
      }
    });
  } catch (error) {
    res.status(500).json({
...
router.post('/gold-attack/stealth', async (req, res) => {
  try {
    ...
    const result = await enhancedGoldAttackSystem.executeEnhancedAttack('STEALTH_CONTINUOUS', customParams);
    ...
    res.json({
      ...
      data: {
        ...
        performance_summary: {
          ...
          success_rate: attackHistory.length > 0 ? 
            (attackHistory.filter(a => a.success).length / attackHistory.length * 100).toFixed(1) + '%' : '0%',
          avg_effectiveness: attackHistory.length > 0 ?
            (attackHistory.reduce((sum, a) => sum + a.effectiveness, 0) / attackHistory.length).toFixed(1) + '%' : '0%'
        }
      }
    });
  } catch (error) {
  ...
  try {
    ...
    for (const [source, data] of marketData) {
      if (data.vulnerability_score && data.vulnerability_score > 60) {
        const suitableVectors = vectors.filter(v => 
          v.target === 'ALL' || v.target === source.toUpperCase() ||
          (source.includes('FSAPI') && v.target === 'FSAPI')
        );
        
        const bestVector = suitableVectors.sort((a, b) => b.effectiveness_rate - a.effectiveness_rate)[0];
    ...
    recommendations.sort((a, b) => (b.vulnerability * b.effectiveness) - (a.vulnerability * a.effectiveness));
    ...
    res.json({
      ...
      data: {
        ...
        fsapi_specific: recommendations.filter(r => r.target.includes('FSAPI'))
      }
    });
  } catch (error) {
    res.status(500).json({
...
router.post('/gold-attack/quick', async (req, res) => {
  try {
    ...
    if (!bestTarget) {
      for (const [source, data] of marketData) {
        if (data.vulnerability_score && data.vulnerability_score > bestVulnerability) {
    ...
    const suitableVectors = vectors.filter(v => 
      v.target === 'ALL' || v.target === (bestTarget || '').toUpperCase() ||
      (bestTarget?.includes('FSAPI') && v.target === 'FSAPI')
    ).filter(v => v.intensity === intensity_preference || v.intensity === 'MAXIMUM');
    ...
    const selectedVector = suitableVectors.sort((a, b) => b.effectiveness_rate - a.effectiveness_rate)[0];
    
    // Execute the attack
    const result = await enhancedGoldAttackSystem.executeEnhancedAttack(selectedVector.name);
