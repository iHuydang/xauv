...
const physicalGoldSellSchema = z.object({
  ...
  goldType: z
    .enum(["SJC_9999", "PNJ_9999", "DOJI_9999", "MIHONG_9999"])
    .default("SJC_9999"),
...
class PhysicalGoldSellSystem {
  ...
  static async executeSellOrder(sellRequest: any): Promise<GoldSellOrder> {
    ...
    const sellPrice = sellRequest.urgentSale
      ? currentPrice * 0.98
      : currentPrice; // Urgent sale có discount 2%
    ...
    console.log(
      `   Weight: ${sellRequest.weight} grams (${weightInTaels.toFixed(2)} taels)`,
    );
...
router.post("/physical-gold/sell", async (req, res) => {
  try {
    ...
    // Kiểm tra inventory
    const inventory = PhysicalGoldSellSystem.getInventory(
      sellRequest.accountId,
    );
    ...
    const currentPrice = PhysicalGoldSellSystem.getCurrentGoldPrice(
      sellRequest.goldType,
    );
    if (sellRequest.minPrice && currentPrice < sellRequest.minPrice) {
      return res.status(400).json({
        ...
        suggestion:
          "Wait for price increase or remove minimum price requirement",
      });
    }

    // Thực hiện lệnh bán
    const sellOrder =
      await PhysicalGoldSellSystem.executeSellOrder(sellRequest);
    ...
    res.json({
      ...
      data: {
        ...
        priceAnalysis: {
          ...
          priceDiscount: sellRequest.urgentSale
            ? "2% urgent sale discount applied"
            : "Market price",
    ...
    res.json({
      ...
      data: {
        ...
        currentPrices: {
          ...
          MIHONG_9999:
            PhysicalGoldSellSystem.getCurrentGoldPrice("MIHONG_9999"),
        },
      },
      timestamp: new Date().toISOString(),
    });
...
router.get("/physical-gold/sell-history", async (req, res) => {
  try {
    ...
    const totalSold = sellHistory.reduce(
      (total, order) => total + order.weight,
      0,
    );
    const totalRevenue = sellHistory.reduce(
...
router.post("/physical-gold/quote", async (req, res) => {
  try {
    ...
    const sellPrice = quoteRequest.urgentSale
      ? currentPrice * 0.98
      : currentPrice;
    const totalValue = sellPrice * weightInTaels;
