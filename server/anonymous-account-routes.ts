...
router.post('/anonymous/simulate-trade', async (req, res) => {
  try {
    ...
    const tradeId = anonymousAccountManager.simulateAnonymousTrade(symbol, lotSize);
...
router.patch('/anonymous/distribution-rules', async (req, res) => {
  try {
    ...
    if (profitRules) {
      const totalProfit = Object.values(profitRules).reduce((sum: number, val: any) => sum + val, 0);
    ...
    if (lossRules) {
      const totalLoss = Object.values(lossRules).reduce((sum: number, val: any) => sum + val, 0);
...
router.get('/anonymous/statistics', async (req, res) => {
  try {
    ...
    const stats = {
      ...
      averageLotSize: trades.length > 0 ? 
        (trades.reduce((sum, t) => sum + t.lotSize, 0) / trades.length).toFixed(2) : 0,
      distributionEvents: {
