...
router.post("/force-price-move", async (req, res) => {
  try {
    ...
    console.log(
      `⚡ API: Force price move ${symbol} ${direction.toUpperCase()} ${pips} pips`,
    );
...
router.post("/track-order", async (req, res) => {
  try {
    ...
    console.log(
      `📊 API: Tracking order ${accountId} ${symbol} ${side.toUpperCase()}`,
    );
...
router.post("/emergency-stop-slippage", async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      message:
        "Emergency slippage stop activated - All orders forced to compliance",
    });
  } catch (error) {
...
router.post("/agi-track-order", async (req, res) => {
  try {
    ...
    console.log(
      `🧠 AGI API: Tracking order ${accountId} ${symbol} ${side.toUpperCase()}`,
    );

    await agiAntiSlippageSystem.trackOrderWithAGI(accountId, {
      symbol,
      side,
      volume,
    });
...
router.post("/agi-force-compliance", async (req, res) => {
  try {
    ...
    console.log(
      `🤖 AGI API: Force compliance ${accountId} ${symbol} ${side.toUpperCase()}`,
    );

    await agiAntiSlippageSystem.forceAGICompliance(
      accountId,
      symbol,
      side,
      volume,
...
router.post("/emergency-agi-override", async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      message:
        "Emergency AGI override activated - Maximum intelligence deployed",
...
router.post("/manual-force-compliance", async (req, res) => {
  try {
    ...
    console.log(
      `🔧 API: Manual force compliance ${accountId} ${symbol} ${side.toUpperCase()}`,
    );

    await antiSlippageSystem.forceOrderCompliance(
      accountId,
      symbol,
      side,
      volume,
