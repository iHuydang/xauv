...
import { formatPrice, formatCurrency, getPriceChangeClass } from '@/lib/trading-utils';
      ...
      <CardContent>
        <Tabs defaultValue="positions" className="h-full">
          <TabsList className="grid w-full grid-cols-2 bg-gray-800">
            <TabsTrigger value="positions" className="text-white data-[state=active]:bg-gray-600">
              Vị thế đang mở ({positions?.length || 0})
            </TabsTrigger>
            <TabsTrigger value="orders" className="text-white data-[state=active]:bg-gray-600">
            ...
            {positions?.length === 0 ? (
              ...
              <div className="space-y-2">
                {positions?.map((position: any) => {
                  ...
                  return (
                    <div key={position.id} className="bg-gray-800 p-3 rounded trading-border border">
                        ...
                        <div className="flex-1">
                          ...
                          <div className="grid grid-cols-2 gap-4 mt-2 text-sm">
                            <div>
                              ...
                              <span className="text-white">{position.volume} lots</span>
                            </div>
                            <div>
                            ...
                            <div>
                              ...
                              <span className="text-white font-mono">
                                {formatPrice(getCurrentPrice(position.symbol, position.type).toString(), position.symbol)}
                              </span>
                            </div>
                            <div>
            ...
            {orders?.length === 0 ? (
              ...
              <div className="space-y-2">
                {orders?.map((order: any) => (
                  <div key={order.id} className="bg-gray-800 p-3 rounded trading-border border">
                    <div className="flex justify-between items-start">
                      <div className="flex-1">
                        <div className="flex items-center gap-2">
                        ...
                        <div className="grid grid-cols-2 gap-4 mt-2 text-sm">
                          <div>
                            <span className="text-gray-400">Khối lượng: </span>
                            <span className="text-white">{order.volume} lots</span>
                          </div>
                          {order.price && (
                            <div>
