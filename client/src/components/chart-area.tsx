...
import { formatPrice, getPriceChangeClass, formatPercentage, getPriceChangeIcon } from '@/lib/trading-utils';
...
export default function ChartArea({ selectedSymbol }: ChartAreaProps) {
  ...
  const priceData = prices[selectedSymbol] || { bid: '0', ask: '0', change: '0', changePercent: '0' };
  ...
  useEffect(() => {
    ...
    if (!window.TradingView && !document.querySelector('script[src*="tradingview"]')) {
        ...
        <div className="p-4 trading-border border-b flex justify-between items-center">
          <div className="flex items-center space-x-4">
            ...
            <div className="flex items-center space-x-2">
              ...
              <span className={`text-sm px-2 py-1 rounded ${changeClass} ${
                parseFloat(priceData.changePercent) >= 0 ? 'bg-green-500/20' : 'bg-red-500/20'
              }`}>
                {changeIcon} {priceData.change} ({formatPercentage(priceData.changePercent)})
              </span>
            </div>
          </div>
          <div className="flex items-center space-x-2">
