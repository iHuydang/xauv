...
import { Activity, DollarSign, TrendingUp, Settings, Play, Pause, RefreshCw } from 'lucide-react';
...
export default function CoinrankingMarketMaker() {
  ...
  const isMarketMakingActive = statusData?.data?.status?.marketMakingActive || false;
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            <Card>
              ...
              <CardContent>
                ...
                <p className="text-xs text-muted-foreground">
                  Max: {formatCurrency(statusData?.data?.status?.config?.maxExposure || 0)}
                </p>
                <Progress 
                  value={((statusData?.data?.status?.totalExposure || 0) / (statusData?.data?.status?.config?.maxExposure || 1)) * 100}
                  className="mt-2"
                />
              </CardContent>
            </Card>
            ...
            <Card>
              ...
              <CardContent>
                ...
                <p className="text-xs text-muted-foreground">
                  Buy: {ordersData?.data?.summary?.buy_orders || 0} | 
                  Sell: {ordersData?.data?.summary?.sell_orders || 0}
                </p>
              </CardContent>
            </Card>
          ...
          {statusData?.data?.recent_prices && (
            <Card>
              <CardHeader>
                ...
                <CardDescription>Live cryptocurrency prices from Coinranking</CardDescription>
              </CardHeader>
              <CardContent>
                <div className="space-y-3">
                  {statusData.data.recent_prices.map((asset: CoinrankingAsset) => (
                    <div key={asset.uuid} className="flex items-center justify-between p-3 border rounded-lg">
                      <div className="flex items-center space-x-3">
                      ...
                      <div className="text-right">
                        <p className="font-bold">{formatCurrency(asset.price)}</p>
          ...
          {goldAnalysis && (
            <Card>
              <CardHeader>
                ...
                <CardDescription>Comprehensive analysis of gold-related crypto assets</CardDescription>
              </CardHeader>
              <CardContent>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
                  <div className="p-4 border rounded-lg">
                    <h4 className="font-medium text-sm text-gray-600">Total Assets</h4>
                    <p className="text-2xl font-bold">{goldAnalysis.data.analysis.total_assets}</p>
                  </div>
                  <div className="p-4 border rounded-lg">
                    <h4 className="font-medium text-sm text-gray-600">Market Cap</h4>
                    <p className="text-2xl font-bold">{formatCurrency(goldAnalysis.data.analysis.total_market_cap)}</p>
                  </div>
                  <div className="p-4 border rounded-lg">
                    <h4 className="font-medium text-sm text-gray-600">Avg Volatility</h4>
                    <p className="text-2xl font-bold">{formatPercentage(goldAnalysis.data.analysis.average_volatility)}</p>
                  </div>
                </div>

                {goldAnalysis.data.analysis.vietnam_gold_equivalent && (
                  <div className="p-4 bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-lg">
                    <h4 className="font-medium mb-2">Vietnam Gold Equivalent (BTC)</h4>
                    <p>BTC Price: {formatCurrency(goldAnalysis.data.analysis.vietnam_gold_equivalent.btc_price_usd)}</p>
                    <p>VND Estimate: {formatNumber(goldAnalysis.data.analysis.vietnam_gold_equivalent.vietnam_estimate, 0)} VND</p>
                  </div>
                )}

                <div className="space-y-3">
                  <h4 className="font-medium">Gold Assets Tracked</h4>
                  {goldAnalysis.data.gold_assets.map((asset: CoinrankingAsset) => (
                    <div key={asset.uuid} className="flex items-center justify-between p-3 border rounded-lg">
                      <div className="flex items-center space-x-3">
                        <div>
                          <p className="font-medium">{asset.symbol}</p>
                        ...
                        <div className="flex items-center space-x-4">
                          ...
                          <div>
                            ...
                            <p className="text-sm text-gray-600">
                              {formatNumber(order.quantity, 6)} @ {formatCurrency(order.price)}
                            </p>
                          </div>
                        </div>
                        <div className="text-right">
                          <p className="font-medium">{formatCurrency(order.market_value || 0)}</p>
                          <p className="text-sm text-gray-600">
          ...
          {positionsData && (
            <Card>
              <CardHeader>
                ...
                <CardDescription>Active positions held by the market maker</CardDescription>
              </CardHeader>
              <CardContent>
                {positionsData.data.positions.length === 0 ? (
                  ...
                  <div className="space-y-3">
                    {positionsData.data.positions.map((position: any) => (
                      <div key={position.asset_uuid} className="flex items-center justify-between p-4 border rounded-lg">
                        <div className="flex items-center space-x-4">
                        ...
                        <div className="text-right">
                          ...
                          <p className="text-sm text-gray-600">
                            {formatNumber(position.quantity, 6)} @ {formatCurrency(position.current_price)}
                          </p>
                          <p className={`text-sm ${position.change_24h >= 0 ? 'text-green-600' : 'text-red-600'}`}>
            ...
            <CardContent className="space-y-6">
              ...
              <div className="flex items-center space-x-2">
                <Switch
                  ...
                  onCheckedChange={(checked) => setConfig({...config, goldFocused: checked})}
                />
                <Label htmlFor="goldFocused">Focus on Gold-Related Assets</Label>
              </div>
